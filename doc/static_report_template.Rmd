...
names(meta_table) <- gsub("_", " ", names(meta_table))
meta_table %>%
  kableExtra::kable(
    "latex",
    booktabs = TRUE,
...
df_synergy %>% 
  kableExtra::kable("latex", caption = "Synergy Score Summary Table") %>%
...
df_synergy_statistic %>%
  kableExtra::kable("latex", caption = "Synergy Score Statistic Table") %>%
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"))
}
```
...
df_sensitive <- synergyfinder_obj$drug_pairs %>% 
  ...
  dplyr::select(
    ...
    dplyr::contains("ri_"),
    "CSS" = "css")    
names(df_sensitive) <- gsub("_", " ", names(df_sensitive))
df_sensitive %>% 
  kableExtra::kable("latex", caption = "Sensitivity Score Summary Table") %>%
  kableExtra::kable_styling(latex_options = c("striped", "HOLD_position"))
```
...
if (any(synergyfinder_obj$drug_pairs$replicate)) {
  df_sensitive_statistic <- synergyfinder_obj$sensitivity_scores_statistics %>% 
    ...
    names(df_sensitive_statistic) <- gsub("_", " ", names(df_sensitive_statistic))
    df_sensitive_statistic %>% 
    # knitr::kable(caption = "Synergy Score Statistic Table", format = "latex")
    kableExtra::kable("latex", caption = "Sensitivity Score Statistic Table") %>%
