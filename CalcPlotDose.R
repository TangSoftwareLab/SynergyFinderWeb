...
{ 
  ...
  for (i in 1:num.pairs) {
    ...
    if (type == "all") {
      ...
      plot(drug.row.model, xlab = paste0("Concentration ", unit.text), 
           ylab = paste0("inhibition (%)"), type = "obs", col = "red",  
           cex = 1.5, pch = 16, xtsty = "base5", ylim=c(-5,105))
      plot(drug.row.model, xlab = paste0("Concentration ", unit.text), 
           ylab = paste0("inhibition (%)"), type = "none", cex = 1.5, add = T, 
           lwd = 3, ylim=c(-5,105))
      par(mar=c(0,0,11,0))
      ...
      print(dose.response.p, vp = viewport(height = unit(0.95, "npc"), 
                                           width = unit(0.53, "npc"), 
                                           just = c("left", "top"),
                                           y = 1, x = 0.48))
      #graphics.off();
      ...
      plot(drug.row.model, xlab = x.lab,  ylab = paste0("inhibition", " (%)"), 
           type = "obs", col = "red", cex = 1.5, pch = 16, xtsty = "base5")
      plot(drug.row.model, xlab = x.lab, ylab = paste0("inhibition", " (%)"), 
           type = "none",  cex = 1.5, add = T, lwd = 3)
      par(mar = c(0, 1, 6, 0))
      title(paste0("Dose-response curve for drug: ", drug.row), cex.main = 1)
      par(mar = c(1.5, 8, 4.5, 5.5))
      ...
      plot(drug.col.model, xlab = x.lab, ylab = paste0("inhibition", " (%)"), 
           type = "obs", col = "red", cex = 1.5, pch = 16, xtsty = "base5")
      plot(drug.col.model, xlab = x.lab, ylab = paste0("inhibition", " (%)"), 
           type = "none", cex = 1.5, add = T, lwd = 3)
      par(mar = c(0, 1, 6, 0))
