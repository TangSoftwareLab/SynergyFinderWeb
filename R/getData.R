transformInputData <- compiler::cmpfun(function(dataOutput, data_typ = NULL){
  ...
  drug.pairs <- data.frame(drug.row = 1:num.pairs, drug.col = 1:num.pairs, 
                          concUnit = 1:num.pairs, PairIndex = 1:num.pairs)
  error_ <- rep(1, num.pairs) # errors initially 1
  warning_ <- rep("", num.pairs) # warnings initially ""
  
  for (i in 1:num.pairs){
    df2 <- dplyr::arrange(dataOutput[dataOutput$PairIndex==pairs_[i],],
                          Conc1, Conc2)
    ...
    tryCatch({
      ...
      if(length(Conc1) * length(Conc2) != length(df2$Response))
        warning_[i] <- paste0("The provided number of responses for ", i,
                              " PairIndex is: <b>",length(df2$Response), 
                              "</b>. However <b>",length(Conc1) * length(Conc2),
                              "</b> responses were expected. Please reconsider <b>",
                              df2$Drug1[1]," & ", df2$Drug2[1],
...
transformInputDataMatrix <- compiler::cmpfun(function(dataOutput, data_typ = NULL){
  ...
  for(i in 1:length(D1)){
    ...
    resp.mat <- matrix(tmpMat[2:nrow(tmpMat), 2:ncol(tmpMat)], 
                      nrow = length(row_names_), ncol = length(col_names_),
    ...
    if(anyNA(resp.mat))
      warning_[i] <- paste0("The dose-response matrix for ", 
                            drug.pairs[i,]$drug.row," and ", 
                            drug.pairs[i,]$drug.col,
                            " has some missing values. ",
                            "Please consider estimation of NA values feature ",
